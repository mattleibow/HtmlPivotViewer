...
namespace Pivot {
     ...
     * visible to the user.
     */
    export class PivotViewer {
        items = <Item[]>[];
        filters = <ItemFilter[]>[];
        facets: FacetMap

        allItemsById = <ItemMap>{} // lets us look up any item in the collection by ID
        ...
        activeItemsArr = <Item[]>[];
        rearrangingItemsArr = <Item[]>[];

        delayedFunction: Function; // anything we need to run at the beginning of the next repaint

        constructor(
            canvas: HTMLElement | JQuery,
            container: HTMLElement | JQuery,
            frontLayer: HTMLElement | JQuery,
            backLayer: HTMLElement | JQuery,
            leftRailWidth: number, rightRailWidth: number,
            inputElmt: HTMLElement | JQuery) {
        }
         ...
         * <dl>
         *   <dt>type</dt>
         *   <dd>string - The type of facet category. Valid types are "String",
         *     "LongString" (which gets treated like String), "Number", "DateTime", and "Link".</dd>
         *   <dt>isFilterVisible</dt>
         *   <dd>bool - Whether the facet shows up in the filter selection
         *     pane and the sort order drop-down</dd>
         *   <dt>isWordWheelVisible</dt>
         *   <dd>bool - Whether the facet category will be accessible via
         *     the search box</dd>
         *   <dt>isMetaDataVisible</dt>
         *   <dd>bool - Whether the facet shows up in the details pane</dd>
         *   <dt>orders</dt>
         *   <dd>optional Array - Allows you to set custom sort orders for String
        ...
        setFacets(newFacets: FacetMap) {
            ...
            for (let facetName in this.facets) {
                if (Helpers.hasOwnProperty(this.facets, facetName)) {
                    let facetData = this.facets[facetName];
                    let orders = facetData.orders;
                    if (orders && orders.length) {
                        // make a new variable scope so we can bind by value
                        ((() => {
                            let orderMap = <NumberMap>{};
                            orders[0].order.forEach((value, index) => {
                                orderMap[value] = index;
                            });
                            facetData.comparator = (a, b) => {
                                let isAOrdered = Helpers.hasOwnProperty(orderMap, a);
                                let isBOrdered = Helpers.hasOwnProperty(orderMap, b);
                                if (isAOrdered) {
                                    if (isBOrdered)
            ...
            this.trigger("hideInfoButton");
            this.trigger("facetsSet", [this.facets]);
        }
         ...
         * </dl>
         */
        setCopyright(legalInfo: LegalInfo) {
            // fire an event so the UI can update
            this.trigger("copyright", [legalInfo]);
        }
        ...
        addItems(newItems: Item[]) {
            ...
            if (!this.items.length && (this.activeItemsArr.length || this.rearrangingItemsArr.length)) {
                // delay it
                this.delayFunction(() => {
            ...
            let waitingItems = 1;
            let actuallyNewItems = <Item[]>[]; // an array of items that were added, not updated
            let onLoad = () => {
                waitingItems--;
                if (!waitingItems) {
                    // all items have loaded, add them to the view
                    this.items = this.items.concat(actuallyNewItems);
                    ...
                    newItems.forEach(this.updateTemplate);
            ...
            newItems.forEach((item: Item) => {
                ...
                if (!item.facets) {
                    item.facets = <FacetListMap>{};
                }
                if (!Helpers.hasOwnProperty(this.allItemsById, id)) {
                    this.allItemsById[id] = item;
                    actuallyNewItems.push(item);
                }

                // refresh the details pane if necessary
                if (this.centerItem === item && this.zoomedIn && this.detailsEnabled) {
                    this.trigger("hideDetails");
                    this.trigger("showDetails", [item, this.facets]);
                }
            });
        ...
        runSearch(searchTerm: string) {
            let results = <string[]>[];
            if (searchTerm) {
                let frontResults = <NumberMap>{};
                let restResults = <NumberMap>{};
                searchTerm = searchTerm.toLowerCase();
                let checkResult = (facetValue: IFacetValue<string | number | Date>) => {
                    let value = Helpers.getFacetString(facetValue);
                    let match = value.toLowerCase().indexOf(searchTerm);
                    let map: NumberMap = null;
                    if (match === 0) {
                        map = frontResults;
                    } else if (match > 0) {
                        map = restResults;
                    ...
                    if (map) {
                        if (Helpers.hasOwnProperty(map, value)) {
                            map[value]++;
                        } else {
                            map[value] = 1;
                        }
                ...
                this.items.forEach((item: Item) => {
                    ...
                    for (let facetName in facets) {
                        if (Helpers.hasOwnProperty(facets, facetName)) {
                            facets[facetName].forEach(checkResult);
                        }
                    }
                    checkResult(<IFacetValue<string>>{ value: item.name });
                });

                for (let array of [frontResults, restResults]) {
                    // convert the map into an array of objects
                    let objects = <{ value: string, count: number }[]>[];
                    for (let value in array) {
                        if (Helpers.hasOwnProperty(array, value)) {
                            objects.push({
                                value: value,
                                count: array[value]
                            });
                    ...
                    objects.sort((a: { count: number }, b: { count: number }) => {
        ...
        zoomedIn: boolean; // whether we're zoomed close enough to need details pane
        //var hoveredBar;
        //var barTemplate;
        //var bars = [];
        ...
        delayFunction(func: Function, delay?: number) {
            if (!delay || delay < 0) {
                if (!this.delayedFunction) {
                    this.delayedFunction = func;
                } else {
                    var otherFunc = this.delayedFunction;
                ...
                this.delayedFunction(() => {
                    this.delayedFunction(func, delay - 1);
                });
            }
        }
        ...
        generateId() {
            ...
            do {
                id = this.nextId++;
            } while (Helpers.hasOwnProperty(this.allItemsById, id.toString()));
            return id.toString();
        }
        ...
        */
        addFilter(filter: (item: Item) => boolean) {
        ...
        */
        runFiltersWithout(filter: ItemFilter) {
            this.removeFilter(filter);
            var result = this.items.filter((item: Item) => {
                return this.filters.every((filter2: ItemFilter) => {
                    return filter2(item);
                });
            });
